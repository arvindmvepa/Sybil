...
class MultiTaskHead(nn.Module):
    def __init__(self, input_dim, num_classes_per_task, num_regression_tasks, hidden_dim=512, in_head_dims=1024):
        ...
        self.shared_layers = nn.Sequential(
            ...
            nn.AdaptiveAvgPool3d(1),   # [B, hidden_dim, 1, 1, 1]
        ...
        for num_classes in num_classes_per_task:
            self.classification_heads.append(
                nn.Sequential(
                    nn.Linear(in_head_dims, in_head_dims // 2),
                    nn.ReLU(),
                    nn.Linear(in_head_dims // 2, num_classes)
                )
            )
        ...
        self.regression_head = nn.Sequential(
            nn.Linear(in_head_dims, in_head_dims // 2),
            nn.ReLU(),
            nn.Linear(in_head_dims // 2, num_regression_tasks)
        )
    
    def forward(self, x):
        cls_token = self.pool(x)["hidden"]

        # Shared features
        shared_features = self.shared_layers(x)
        shared_features = torch.cat([cls_token, shared_features], dim=-1)
...
def main():
    ...
    model = MultiTaskHead(
        ...
        in_head_dims=args.in_head_dims
    ).to(device)
