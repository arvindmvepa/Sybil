...
def evaluate_model(model, dataloader, device, debug_limit=None):
    ...
    with torch.no_grad():
        for index, batch in enumerate(tqdm(dataloader, desc="Evaluating")):
            labels = batch['classification_labels'].to(device)
            
            outputs = model.mod_predict(batch['serie'])
...
def main():
    ...
    parser.add_argument('--save_dir', default='./eval4_results', help='Directory to save results')
    parser.add_argument('--model', default='sybil_ensemble', help='sybil model to use')
    parser.add_argument('--debug_limit', default=100, help='Number of iterations before stopping')
